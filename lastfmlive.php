...
class LastFM_LiveRecentTracks extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		echo $this->fetchRecentTracks($instance['username'], $tracklimit);
		echo $after_widget;
	}
	...
	function fetchRecentTracks($user, $limit){
		if($user == "" || $limit == ""){
			return "";
		}
	...
	// 	exit();
		...
		if(is_array($recent_tracks['recenttracks']['track'])){
			...
			while($t > $i){
				...
				$time = $this->fuzzytime($track['date']['uts'],  $track['@attr']['nowplaying']);
				if($time !== false){
	...
	function fuzzytime($datefrom , $is_playing=false){

		$dateto = time();
		$difference = $dateto - $datefrom;

		if($difference <= 0 && $is_playing){
			return "Listening now..";		//currently listening
		} else if ($difference < 15 && !$is_playing){
			return false;
		} else if($difference < 60){
			return $difference . ' second' . ( $difference > 1 ? 's' : '' ).' ago';
		} else if( $difference < 60*60 ){
			$ago_seconds = $difference % 60;
			$ago_seconds = ' and '.$ago_seconds.' second'.$ago_seconds > 1?'s':''.' ago';
			$ago_minutes = floor( $difference / 60 );
			return $ago_minutes . ' minute'.$ago_minutes > 1?'s ':' '.$ago_seconds>0?$ago_seconds:'ago';
		} else if ( $difference < 60*60*24 ){
			$ago_hours = floor( $difference / ( 60 * 60 ) );
			return  $ago_hours. ($ago_hours > 1 ? ' hours' : ' hour').' ago';
